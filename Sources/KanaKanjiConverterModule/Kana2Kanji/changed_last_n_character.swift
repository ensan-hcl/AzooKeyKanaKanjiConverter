...
extension Kana2Kanji {
    ...
    func kana2lattice_changed(_ inputData: ComposingText, N_best: Int, counts: (deleted: Int, added: Int), previousResult: (inputData: ComposingText, nodes: Nodes)) async throws -> (result: LatticeNode, nodes: Nodes) {
        // (0)
        ...
        // (2)
        let addedNodes: [[LatticeNode]] = await self.dicdataStore.getAddedNodes(inputData: inputData, count: count, commonCount: commonCount)
...
private extension DicdataStore {
    func getAddedNodes(inputData: ComposingText, count: Int, commonCount: Int) -> [[LatticeNode]] {
        (0..<count).map {(i: Int) in
            self.getLOUDSDataInRange(inputData: inputData, from: i, toIndexRange: max(commonCount, i) ..< count)
        }
    }
}
